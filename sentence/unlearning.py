...
def main():
    ...
    model_weights = torch.load('./33efa5633e63b4d0a81613b2b4dcb5e56f864ab3/pytorch_model.bin', map_location='cpu')
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for batch_idx, (teacher_batch, student_batch) in enumerate(progress_bar):
            ...
            loss = 3.0 * ce_loss + 0.01 * distill_loss + 0.001 * con_loss
            loss.backward()
        ...
        if dev_clean_acc > best_dev_acc:
            torch.save(model.state_dict(), os.path.join('fine-tuning', f"pytorch_model.pth"))
